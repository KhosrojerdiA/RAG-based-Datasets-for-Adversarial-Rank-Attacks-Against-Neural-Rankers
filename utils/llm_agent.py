...
def clean_json_response(response_content):
    ...
    try:
        ...
        # Convert inch symbols (") to 'in' for valid parsing
        json_str = re.sub(r'(\d+)½”', r'\1.5 in', json_str)  # Convert 1½” → 1.5 in
        json_str = re.sub(r'(\d+)¼”', r'\1.25 in', json_str)  # Convert 2¼” → 2.25 in
        json_str = re.sub(r'(\d+)”', r'\1 in', json_str)  # Convert 2” → 2 in

        # Convert ranges properly: "4” - 8”" → "4 in - 8 in"
        json_str = re.sub(r'(\d+) in\s*-\s*(\d+) in', r'\1 in - \2 in', json_str)

        # Fix improperly formatted colons in key-value pairs
        json_str = re.sub(r'(\d+ in\s*[-\s]*\d+ in rock coverage):', r'"\1":', json_str)
        ...
        # Convert improperly quoted keys to proper JSON format
        json_str = re.sub(r'([{,]\s*)([A-Za-z_][\w-]*)(\s*:\s*)', r'\1"\2"\3', json_str)

        # Try parsing JSON (use JSON5 for relaxed parsing)
        try:
